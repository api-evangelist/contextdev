...
- **Note:** unreachable URLs return `400` `WEBSITE_ACCESS_ERROR`.
...
- **When:** you know a domain or work email ahead of when you'll need it (CRM import, signup form) and want to warm the cache so the later `/brand/retrieve` lands sub-second.
- **Takes:** `type: "brand"` (**required**), `identifier.domain` or `identifier.email` (exactly one required; both or neither returns **400**), `timeoutMS`. **Gives:** `{ status, message, type, domain, key_metadata }`.
